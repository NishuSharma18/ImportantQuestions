...
for(auto c1: coins){
if(amt-c1 >=0) ans  = min(ans + 0LL,func(amt-c1,coins)+1LL);  
}
return dp[amt]= ans;
}
...
int ct = func(amount,coins);
return ct == INT_MAX ? -1 : ct;}

// Coin Change 2 -->> 
int dp1[305][5005];

int func2(int i,int amt,vector<int>& coins){
int ways =0;
if(amt == 0) return 1;
if(i<0) return 0;
if(dp1[i][amt]!= -1) return dp1[i][amt];

for (int coin_amt = 0; coin_amt <= amt ; coin_amt+= coins[i]){
ways += func2(i-1,amt- coin_amt,coins);
}

return dp1[i][amt] = ways;
}

int change(int amount, vector<int>& coins) {
memset(dp1,-1,sizeof(dp1));
int res = func2(coins.size()-1 ,amount,coins);
return res;        
}
