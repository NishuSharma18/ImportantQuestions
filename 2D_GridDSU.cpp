...
void solve(){
  ...
  for(int row = 0; row < n ; row++) {
    for(int col = 0; col <m; col++) {
      ...
      for(int ind = 0; ind < 4; ind++) {
          ...
          if(isValid(newr,newc,n,m) && grid[newr][newc] == 1){
                  int nodeNo = convert(row,col,m);
                  int adjNodeNo = convert(newr,newc,m);
                  ds.unite(nodeNo, adjNodeNo);
              }
          }
      }
  }
   ...
   for(int row = 0; row < n ; row++) {
      for (int col = 0; col <m; col++){
        if(grid[row][col]==0) continue;
        ll val = convert(row,col,m);
        st.insert(ds.find(val));
       }   
    }
      ...
      for (int col = 0; col <m; col++) {
          ...
          for (int ind = 0; ind < 4; ind++) {
              ...
              if (isValid(newr,newc,n,m)) {
                  if (grid[newr][newc] == 1) {
                      components.insert(ds.find(convert(newr,newc,m)));
                  }
              }
          }
